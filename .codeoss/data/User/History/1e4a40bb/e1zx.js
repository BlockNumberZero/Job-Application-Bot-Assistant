...
function callGeminiForRejection(sender, subject, body, pendingCompanyNames) {
  ...
  const prompt = `You are analyzing a job application email. Determine:
...
RULES:
- A rejection means the company will NOT proceed with the candidate.
- Acknowledgment emails ("we received your application") are NOT rejections.
- Interview invitations are NOT rejections.
- Job alert emails from Indeed, LinkedIn etc. are NOT rejections.
- Subtle German rejections count too: "andere Bewerber entsprechen besser", "nicht weiter berücksichtigen", "absagen müssen", "nicht weiter verfolgen", "bedauern mitteilen".
- Use fuzzy matching — company name in email may differ slightly from list.
- If rejection but no company matches, return null for matchedCompany.
- If not a rejection, return false for isRejection.
...
latestMessage.getPlainBody();
  ...
  // Find best fuzzy match in pending list
  const matchedEntry = findBestCompanyMatch(result.matchedCompany, pendingCompanies);
