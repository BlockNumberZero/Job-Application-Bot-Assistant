...
function processRejectionEmails() {
  ...
  for (const thread of threads) {
    ...
    Logger.log(`Processing thread: "${subject}" from "${sender}"`);
    Logger.log(`Pending companies count: ${pendingCompanies.length}`);
    ...
    for (let c = 0; c < pendingCompanies.length; c++) {
      ...
      if (regex.test(contentToSearch)) {
        Logger.log(`MATCH FOUND: "${cName}" matched thread "${subject}"`);
    ...
    if (!matchedCompany) {
      Logger.log(`NO MATCH for thread: "${subject}"`);
    }
...
function updateSankeyData() {
  ...
  if (output.length > 0) sankeySheet.getRange(2, 1, output.length, 4).setValues(output);
}
...
function updateGeoData() {
  ...
  if (output.length > 0) geoSheet.getRange(2, 1, output.length, 4).setValues(output);
}
function debugRejectionSearch() {
  const timezone = CONFIG.TIMEZONE;
  const since = new Date(new Date().getTime() - 14 * 24 * 60 * 60 * 1000);
  const sinceFormatted = Utilities.formatDate(since, timezone, "yyyy/MM/dd");

  // Test 1: search for WattFox directly by sender
  const q1 = `after:${sinceFormatted} from:personio.de`;
  const r1 = GmailApp.search(q1, 0, 10);
  Logger.log(`Test 1 (personio sender): ${r1.length} threads`);
  r1.forEach(t => Logger.log(`  - ${t.getMessages()[0].getSubject()}`));

  // Test 2: search for Power Consulting by sender
  const q2 = `after:${sinceFormatted} from:indeed.com subject:"Power Consulting"`;
  const r2 = GmailApp.search(q2, 0, 10);
  Logger.log(`Test 2 (indeed + Power Consulting): ${r2.length} threads`);
  r2.forEach(t => Logger.log(`  - ${t.getMessages()[0].getSubject()}`));

  // Test 3: log the full query JABA actually builds
  const rejectionKeywordsEN = [
    '"unfortunately"', '"not moving forward"', '"not to move forward"',
    '"other candidates"', '"careful consideration"', '"careful review"',
    '"we have decided"', '"we\'ve decided"', '"we will not be moving"',
    '"decided not to move"'
  ];
  const rejectionKeywordsDE = [
    '"leider"', '"absage"', '"andere Kandidaten"',
    '"nicht weiterverfolgen"', '"keinen positiven Bescheid"',
    '"nicht berücksichtigen"', '"haben wir uns entschieden"',
    '"Auswahlprozess"', '"Auseinandersetzung"', '"Auswahlverfahren"',
    '"nicht weiter im"', '"entschieden, Ihre Bewerbung"',
    '"Bewerbende entschieden"', '"anderweitig besetzt"',
    '"keine Weiterverfolgung"'
  ];
  const allKeywords = [...rejectionKeywordsEN, ...rejectionKeywordsDE].join(' OR ');
  const fullQuery = `after:${sinceFormatted} (${allKeywords}) -label:bot-rejections-processed`;
  Logger.log(`Full query length: ${fullQuery.length} chars`);
  Logger.log(`Full query: ${fullQuery}`);

  // Test 4: run the actual full query
  const r4 = GmailApp.search(fullQuery, 0, 30);
  Logger.log(`Test 4 (full query): ${r4.length} threads found`);
  r4.forEach(t => Logger.log(`  - ${t.getMessages()[0].getSubject()}`));
}
function debugMatchingStep() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheets = ss.getSheets();
  const timezone = CONFIG.TIMEZONE;

  // Step 1: Log all pending companies JABA is searching for
  Logger.log("=== PENDING COMPANIES ===");
  sheets.forEach(sheet => {
    const sheetName = sheet.getName();
    if (sheetName !== "Sankey_Data" && sheetName !== "Geo_Data" && /\d{4}/.test(sheetName)) {
      const data = sheet.getDataRange().getValues();
      for (let i = 1; i < data.length; i++) {
        const status = data[i][5] ? data[i][5].toString().trim().toLowerCase() : "";
        const companyName = data[i][1] ? data[i][1].toString().trim() : "";
        if (companyName && status !== "rejected" && status !== "ignored" && status !== "i withdrew") {
          const coreName = companyName
            .split(" (")[0]
            .replace(/\s+(GmbH|AG|SE|KG|OHG|UG|Ltd|Inc|Corp|LLC|SAS|BV|NV|AB)\.?$/i, '')
            .trim();
          Logger.log(`Sheet: ${sheetName} | Company: "${companyName}" | CoreName: "${coreName}" | Length: ${coreName.length}`);
        }
      }
    }
  });

  // Step 2: Test matching against WattFox email content
  Logger.log("=== MATCHING TEST: WattFox ===");
  const wattfoxContent = "wattfox-jobs@m.personio.de Deine Bewerbung als AI & Marketing Automation Manager (m/w/d) bei WattFox Hallo Rey, noch einmal vielen Dank WattFox GmbH";
  
  // Step 3: Test matching against Power Consulting email content  
  Logger.log("=== MATCHING TEST: Power Consulting ===");
  const powerContent = "noreply@indeed.com An update on your application from Power Consulting & Low Code GmbH Hallo Rey Chancahuana vielen Dank Powerberatung & Low Code GmbH";

  // Test each pending company against both email contents
  sheets.forEach(sheet => {
    const sheetName = sheet.getName();
    if (sheetName !== "Sankey_Data" && sheetName !== "Geo_Data" && /\d{4}/.test(sheetName)) {
      const data = sheet.getDataRange().getValues();
      for (let i = 1; i < data.length; i++) {
        ...
        if (companyName && status !== "rejected" && status !== "ignored" && status !== "i withdrew") {
          ...
          if (regex.test(wattfoxContent)) {
            Logger.log(`WATTFOX MATCH: "${companyName}" (core: "${coreName}") matched!`);
          }
          if (regex.test(powerContent)) {
            Logger.log(`POWER MATCH: "${companyName}" (core: "${coreName}") matched!`);
          }
        }
      }
    }
