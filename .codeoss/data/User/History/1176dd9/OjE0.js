...
Gemini API Helper
*/
function getGeminiKey() { return getScriptProperty('GEMINI_API_KEY'); }
...
function callGeminiForRejection(sender, subject, body, pendingCompanyNames) {
  ...
  // Truncate body to save tokens — first 1500 chars contain the rejection signal
  const truncatedBody = body.substring(0, 1500);

  // Limit company list to 150 entries to avoid token overflow
  const companyList = pendingCompanyNames.slice(0, 150).join('\n');

  const prompt = `You are analyzing a job application email. Your task is to determine:
1. Is this a job application rejection email?
2. If yes, which company from the provided list most likely sent it?
...
PENDING COMPANIES LIST (match against these exactly):
${companyList}

RULES:
- A rejection email is one where the company says they will NOT proceed with the candidate.
- Acknowledgment emails ("we received your application") are NOT rejections.
- Interview invitations are NOT rejections.
- Use fuzzy matching — the company name in the email may differ slightly from the list (e.g. "Rhenus Gruppe" matches "Rhenus Warehousing Solutions Global GmbH & Co. KG").
- If the email is a rejection but no company in the list matches, return null for matchedCompany.
- If this is not a rejection, return false for isRejection.

Respond ONLY with valid JSON, no markdown, no explanation:
{"isRejection": true, "matchedCompany": "Exact company name from list"}
or
{"isRejection": false, "matchedCompany": null}`;
...
function processRejectionEmails() {
  ...
  // 4. Broad Gmail search — no keyword filtering, rely on Gemini instead
  const query = `after:${sinceFormatted} -label:${labelName} in:inbox`;
  ...
  let lastProcessedDate = sinceDate;
  ...
  for (const thread of threadsToProcess) {
    ...
    // Call Gemini
    const result = callGeminiForRejection(sender, subject, body, pendingNames);

    if (!result) {
      Logger.log(`Gemini returned null — skipping.`);
      // Update date tracker even on failure
      if (messageDate > lastProcessedDate) lastProcessedDate = messageDate;
      Utilities.sleep(300);
      continue;
    }
    ...
    if (result.isRejection && result.matchedCompany) {
      // Find exact match in pending list
      const matchedEntry = pendingCompanies.find(p =>
        p.name === result.matchedCompany
      );
    ...
    if (messageDate > lastProcessedDate) lastProcessedDate = messageDate;
    Utilities.sleep(300); // Avoid Gemini rate limits
  }

  // 7. Update timestamp
  const processedAll = threadsToProcess.length < MAX_PER_RUN;
  if (processedAll) {
    // All caught up — set timestamp to now
    props.setProperty('LAST_REJECTION_SCAN', new Date().toISOString());
  } else {
    // More emails remain — save last processed date so next run continues from here
    props.setProperty('LAST_REJECTION_SCAN', lastProcessedDate.toISOString());
  }
