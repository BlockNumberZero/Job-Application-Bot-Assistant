...
function updateGeoData() {
    ...
    if (sheetName.includes("2026")) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (city && city.trim() !== "" && city !== "Remote") {
          ...
          // Correlate the full funnel using binary columns
          geoCounts[key].applications += 1;
          if (data[i][10] === 1) geoCounts[key].hrInterviews += 1;    // Col K
          if (data[i][11] === 1) geoCounts[key].firstInterviews += 1; // Col L
          if (data[i][12] === 1) geoCounts[key].secondInterviews += 1;// Col M
          if (data[i][13] === 1) geoCounts[key].thirdInterviews += 1; // Col N
          if (data[i][14] === 1) geoCounts[key].fourthInterviews += 1;// Col O
          if (data[i][15] === 1) geoCounts[key].offers += 1;          // Col P
          if (data[i][16] === 1) geoCounts[key].ignored += 1;         // Col Q
          if (data[i][17] === 1) geoCounts[key].rejections += 1;      // Col R
        }
      }
    }
  });
  ...
  // NEW HEADERS: Correlating geographical data with the full job funnel
  geoSheet.getRange(1, 1, 1, 12).setValues([["Month", "City", "Country", "Applications", "HR Interviews", "1st Interviews", "2nd Interviews", "3rd Interviews", "4th Interviews", "Offers", "Ignored", "Rejections"]]);
  ...
  const output = Object.keys(geoCounts).map(key => {
    ...
    return [
      ...
      city, 
      country, 
      geoCounts[key].applications,
      geoCounts[key].hrInterviews,
      geoCounts[key].firstInterviews,
      geoCounts[key].secondInterviews,
      geoCounts[key].thirdInterviews,
      geoCounts[key].fourthInterviews,
      geoCounts[key].offers,
      geoCounts[key].ignored,
      geoCounts[key].rejections
    ];
  });

  if (output.length > 0) {
    geoSheet.getRange(2, 1, output.length, 12).setValues(output);
  }
}