...
function updateGeoData() {
    ...
    if (sheetName.includes("2026")) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (city && city.trim() !== "" && city !== "Remote") {
          ...
          // Correlate the conditions using binary columns
  ...
  geoSheet.getRange(1, 1, 1, 8).setValues([["Month", "City", "Country", "Applications", "HR Interviews", "1st Interviews", "Offers", "Rejections"]]);
  ...
  if (output.length > 0) {
    ...
    geoSheet.getRange(2, 1, output.length, 8).setValues(output);
  }
}