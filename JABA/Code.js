...
function onEdit(e) {
  ...
  if ((col === 6 || (col >= 10 && col <= 18)) && row > 1) {
    ...
    SpreadsheetApp.flush(); // Force writes to complete before reading
    updateSankeyData();
    //CRITICAL INTEGRATION: Update Geo Data tab whenever ANY status changes
    updateGeoData();
    SpreadsheetApp.flush(); // Ensure updates to data tabs are committed
  }
}
...
9. Preserve hyphens only in compound words (e.g. "SaaS-Produkt", "e-mail").`;
    ...
    // --- CRITICAL INTEGRATION: Update Data Tabs ---
    SpreadsheetApp.flush(); // Force writes to complete before reading
    Utilities.sleep(1000); // Give the Google Sheets backend a moment to catch up
    updateSankeyData();
    updateGeoData();
    SpreadsheetApp.flush(); // Ensure writes to data tabs are committed before returning
...
function processGmailApplications() {
  ...
  try {
    ...
    // Update data tabs if new applications were registered
    if (registered > 0) {
      SpreadsheetApp.flush(); // Force writes to complete before reading
      Utilities.sleep(1000); // Give the Google Sheets backend a moment to catch up
      updateSankeyData();
      updateGeoData();
      SpreadsheetApp.flush(); // Ensure writes to data tabs are committed
    }
...
function processRejectionEmails() {
  ...
  // 1. Get pending applications from ALL monthly sheets
  let pendingCompanies = [];
  sheets.forEach(sheet => {
    const sheetName = sheet.getName();
    // Match sheets with a 4-digit year to capture all monthly tabs (e.g. "Mar 2026")
...
const query = `after:${sinceFormatted} (${allKeywords}) -label:${labelName}`;
  ...
  if (regex.test(contentToSearch)) {
    ...
    if (matchedCompany) {
      // 5. Update the Sheet
      matchedCompany.sheet.getRange(matchedCompany.rowIndex, 6).setValue("Rejected"); // Column F = Status
      matchedCompany.sheet.getRange(matchedCompany.rowIndex, 20).setValue(botRejectionMark); // Column T = Email Rejection
  ...
  if (rejectionsFound > 0) {
    // Update data tabs if new rejections were logged
    SpreadsheetApp.flush(); // Force writes to complete before reading
    Utilities.sleep(1000); // Give the Google Sheets backend a moment to catch up
    updateSankeyData();
    updateGeoData();
...
function updateSankeyData() {
  ...
  sheets.forEach(sheet => {
    ...
    if (sheetName.includes("2026")) {
      ...
      // Force explicit range fetch instead of relying on cached getDataRange()
